...
def get_help():
    print(
        "run-virus  \t\t-> run telegram bot virus\n"
...
def get_prev_command(command_idx):
    with open('cmds_history.txt') as file:
        ...
        if commands_history:
            if command_idx == 2:
                print(f"\r→ {relative_path} @ {commands_history[-command_idx]}", end="\n`")
...
def get_and_run_command(relative_path, command=None):
    while True:
        # get input from user
        if not command:
        ...
        elif command == "run-virus":
            os.system("python tel_bot.py")
        elif command == "prev":
            command = get_prev_command(2)
            get_and_run_command(relative_path=relative_path, command=command)
        ...
        # reseting the command to get a new one
        command = None
...
if __name__ == "__main__":
    ...
    # Get and Run command
    get_and_run_command(relative_path=relative_path, command=None)